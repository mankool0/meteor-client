            ...
            if (info.hasPresets() && !searchBar.get().isEmpty()) {
                for (HudElementInfo<?>.Preset preset : info.presets) {
                    ...
                    if (Utils.searchTextDefault(title, searchBar.get(), false))
                        grouped.computeIfAbsent(info.group, unused1 -> new ArrayList<>()).add(new Item(title, info.description, preset));
                }
            } else if (Utils.searchTextDefault(info.title, searchBar.get(), false))
                grouped.computeIfAbsent(info.group, unused2 -> new ArrayList<>()).add(new Item(info.title, info.description, info));
        }
