...
public class WireframeEntityRenderer {
    ...
    public static void render(Render3DEvent event, Entity entity, double scale, Color sideColor, Color lineColor, ShapeMode shapeMode) {
        ...
        renderer.render(state, matrices, MyVertexConsumerProvider.INSTANCE, LightTexture.FULL_BRIGHT);
        matrices.popPose();
    }

    private static class MyVertexConsumerProvider implements MultiBufferSource {
        ...
        public VertexConsumer getBuffer(RenderType layer) {
            if (layer instanceof RenderType.CompositeRenderType composite && composite.state.outputState == RenderStateShard.ITEM_ENTITY_TARGET) {
                return NoopVertexConsumer.INSTANCE;
            }

            return buffers.computeIfAbsent(layer, unused1 -> new MyVertexConsumer());
        }
    }
